...
CREATE TABLE IF NOT EXISTS `clientes` (
 ...
 `Correo_Cliente` varchar(30) DEFAULT NULL, `Fecha_Ingreso` date DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `factura_ventas` (
  `Numero_Factura_Venta` int(11) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `ventas` (
  ...
  KEY `fk_Ventas_Clientes_idx` (`Clientes_Cedula_Cliente`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
--
ALTER TABLE `ventas`
  ADD CONSTRAINT `fk_Ventas_Clientes` FOREIGN KEY (`Clientes_Cedula_Cliente`) REFERENCES `clientes` (`Cedula_Cliente`) ON DELETE NO ACTION ON UPDATE NO ACTION;
