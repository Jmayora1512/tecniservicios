...
CREATE TABLE IF NOT EXISTS `inventario`.`Ventas` (
  ...
  `Clientes_Cedula_Cliente` INT NOT NULL,
  `Forma_Pago_idForma_Pago` INT NOT NULL,
  PRIMARY KEY (`idVentas`),
  INDEX `fk_Ventas_Clientes_idx` (`Clientes_Cedula_Cliente` ASC),
...
CREATE TABLE IF NOT EXISTS `inventario`.`Proveedores` (
  ...
  `Direccion_Comercio` VARCHAR(45) NOT NULL,
  `Fecha_Ingreso` DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS `inventario`.`Compras` (
  ...
  `Proveedores_RIF` VARCHAR(12) NOT NULL,
  PRIMARY KEY (`idCompras`),
  INDEX `fk_Compras_Proveedores1_idx` (`Proveedores_RIF` ASC),
  CONSTRAINT `fk_Compras_Proveedores1`
    FOREIGN KEY (`Proveedores_RIF`)
    REFERENCES `inventario`.`Proveedores` (`RIF`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `inventario`.`Factura_Compras` (
  ...
  `Fecha_Factura_Compra` VARCHAR(45) NOT NULL,
